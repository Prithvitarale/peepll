...
class TargetValAccuracyCallback(Callback):
    def __init__(self, min_val_accuracy, max_val_accuracy):
        ...
        self.min_val_accuracy = 0.6
        self.max_val_accuracy = 0.7
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.train_mode == 0:
                total_loss = reconstruction_loss + tf.reduce_mean(0.00396*(kl_loss))
            else:
...
def identify_from_memory_scalable_forkl(model, queries, p_batch, confN, ms_scores):
    ...
    for q, query in enumerate(queries):
        ...
        if len(matching_indices) == 0:
            ...
            # Get z_means for memory of predicted class
...
def communication(
    ...
    if print_stats == 1:
        ...
        print(f"print_stats executed in: {elapsed_time:.4f} seconds")
...
with tf.device('/GPU:0'):
    ...
    if not loaded_weights:
        ...
        for i, mi in enumerate(modelN):
            ...
            mi.save_weights(f'/work/pi_hava_umass_edu/prithvi/save_mimgnet/{i+1}_img_10agentsrerr_5_{config.figure_i}.h5')
    model_list = modelN
    ...
    for task_id, x_train in enumerate(task_wise_x):
        ...
        for task_epoch in range(1):
            ...
            for i in range(training_len):
                ...
                with open(f"./finalruns_imgnet_reproduce_majority_correct8_{config.figure_i}.pkl", 'wb') as f:
                ...
                print("seed=1, rm11c")
                with open(f"./finalruns_imgnet_reproduce_majority_correct8_data_{config.figure_i}.pkl", 'wb') as f:
